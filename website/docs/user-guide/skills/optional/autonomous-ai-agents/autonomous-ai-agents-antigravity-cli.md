...
- A print run writes only where it is allowed. Pass `--add-dir <dir>` — an
  absolute target and a bare filename inside that dir both succeed with the flag
  alone — or add an allow-rule / `--dangerously-skip-permissions`. Without one of
  those, a write to your target is auto-denied (`a tool required the "command"
  permission that headless mode cannot prompt for, so it was auto-denied`) and
  nothing lands there; a bare relative filename can instead land in the app's own
  artefacts dir (`~/.gemini/antigravity-cli/scratch/`) with the run still
  reporting success, so verify the path on disk rather than the reply.
...
- Give a print run a writable target: `--add-dir <repo>` (or an allow-rule /
  `--dangerously-skip-permissions`). Without it a write to your target is
  auto-denied, and a bare relative filename can land under
  `~/.gemini/antigravity-cli/scratch/` instead — with the run still reporting
  success. `workdir=` on the `terminal` tool is not enough on its own.
